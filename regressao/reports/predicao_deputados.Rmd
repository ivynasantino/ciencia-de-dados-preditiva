...
```{r, warning=FALSE, message=FALSE}
test$gastos <- test$total_receita - test$total_despesa

test$deputado <- ifelse(test$ocupacao == "DEPUTADO", TRUE, FALSE)
test$deputado <- as.factor(test$deputado)

test$ens_superior <- ifelse(test$grau == "SUPERIOR COMPLETO", TRUE, FALSE)
test$ens_superior <- as.factor(test$ens_superior)
...
```{r, warning=FALSE, message=FALSE}
predicao <- predict(modelo.ada.novo, test)
submissao$situacao <- predicao
