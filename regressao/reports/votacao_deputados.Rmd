...
```{r, message=FALSE, warning=FALSE}
modelo_tot_06 <- lm(votos ~ ., data = eleicao_2006 %>% 
                   select(-cargo, 
                          -nome, 
...
```{r, message=FALSE, warning=FALSE}
modelo_tot_10 <- lm(votos ~ ., data = eleicao_2010 %>% 
                   select(-cargo, 
                          -nome, 
                          -sequencial_candidato))
```
...
correlacao forte com relacao a votos: tot_desp = 0.66, pes_juridica = 0.59, tot_receita = 0.64, med_receita = 0.48

todas possuem correlação forte entre si, então escolhi tot_desp para construir o modelo já tem maior correlação entre 
escolher = tot_desp
...
```{r, message=FALSE, warning=FALSE}
#2006
modelo_06 <- lm(votos ~ total_despesa, data = eleicao_2006 %>% 
